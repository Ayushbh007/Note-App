<script lang="ts">
  ...
  const typeStyles = {
    success:
      "bg-emerald-500/95 text-white border border-emerald-400/40 shadow-lg shadow-emerald-500/30",
    error:
      "bg-rose-500/95 text-white border border-rose-400/40 shadow-lg shadow-rose-500/30",
    info: "bg-indigo-500/95 text-white border border-indigo-400/40 shadow-lg shadow-indigo-500/30",
  };
</script>

<div
  class="fixed bottom-4 right-4 z-50 flex items-center gap-3 px-4 py-3 rounded-xl backdrop-blur {typeStyles[
    type
  ]} animate-slide-up"
  role="alert"
>
