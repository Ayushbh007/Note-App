...
>
	<div class="flex items-start justify-between mb-3">
		<div class="flex-1">
			<div class="flex items-center gap-2">
				{#if note.pinned}
					<span class="inline-flex items-center justify-center w-7 h-7 rounded-full bg-emerald-500 dark:bg-lime-300 text-white dark:text-slate-900 font-bold shadow">
		...
		>
			<svg
				class="w-5 h-5 {note.pinned ? 'text-emerald-500 dark:text-lime-300' : 'text-slate-400 dark:text-slate-300'}"
				fill="currentColor"
				viewBox="0 0 20 20"
			>
				<path d="M10 2L3 7v11h4v-6h6v6h4V7l-7-5z" />
	...
	<p class="text-slate-600 dark:text-slate-100 mb-5 whitespace-pre-wrap break-words leading-relaxed">{note.content}</p>
