<script lang="ts">
	...
	let filteredCount = $derived(
		$debouncedSearch 
			? $notes.filter(n => n.title.toLowerCase().includes($debouncedSearch.toLowerCase())).length
			: total
	);
...
{#if total > 0}
	<div class="mb-4 text-sm text-slate-500 dark:text-teal-100/70">
		{#if isFiltered}
			...
			<span class="text-slate-400 dark:text-teal-100/50">(filtered from {total})</span>
		{:else}
