...
export const api = {
  ...
  async getNote(id: string): Promise<Note> {
    const response = await fetchWithTimeout(`${getBaseUrl()}/notes/${id}`);
  ...
  async createNote(note: Omit<Note, "id" | "createdAt">): Promise<Note> {
    ...
    const response = await fetchWithTimeout(`${getBaseUrl()}/notes`, {
      method: "POST",
      headers: {
  ...
  async updateNote(id: string, note: Partial<Omit<Note, "id">>): Promise<Note> {
    const body = {
      ...note,
